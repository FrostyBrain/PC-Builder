...
public class RGBAttach : MonoBehaviour
{
    [SerializeField]
    private GameObject RGBMarker;
    [SerializeField]
    private OVRGrabbable GrabbableRef;
    private string currentPartName= "RGB";

    public void OnTriggerEnter(Collider other)
    {
        if (other.tag == currentPartName&&currentPartName== "RGB")
        {
            //put part colliding with this collider in position/rotation of endmarker
            //Create fixed joint to fuse part to socket
            
            currentPartName = "bluray";
            Debug.Log("Things should be working");

            other.gameObject.transform.rotation = RGBMarker.transform.rotation;

            other.gameObject.transform.parent = RGBMarker.transform;
            //other.gameObject.transform.localPosition = new Vector3(-0.01417515f, 0.01416525f, -0.7377535f);
            other.gameObject.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
        ...
        else if (other.tag == currentPartName&&currentPartName=="bluray")
        {
